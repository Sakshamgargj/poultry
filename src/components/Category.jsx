...
export default function CategorySidebar() {
  ...
  return (
    <div className="flex pt-4 flex-col md:flex-row min-h-screen">
      ...
      >
        <h2 className="text-lg text-center font-mono text-yellow-800 mb-4">Latest Category</h2>
      ...
      <h2 className="text-2xl font-mono text-yellow-700 border-b-2 border-gray-400 border-dotted pb-2 mb-4">{selectedCategory.category}</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {selectedCategory.subcategories.map((sub, index) => (
            <div key={index} className="rounded-lg p-4 shadow-md hover:shadow-lg transition-all bg-white">
              <img src={sub.image} alt={sub.image} className="w-full h-40 object-cover rounded-md" />
              <p className="mt-3 font-mono text-center text-yellow-800">{sub.title}</p>
            </div>
          ))}
        </div>
      </div>
